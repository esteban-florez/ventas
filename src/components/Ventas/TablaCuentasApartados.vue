  ...
  <section>
    <div class="columns">
      ...
      <div class="column is-flex is-justify-content-end">
        <b-button type="is-primary" tag="a" :href="printHref" @click="cargarRegistrosFiltrados" target="__blank"
    ...
    <b-table class="box" :data="datos" :paginated="isPaginated" :per-page="perPage" :current-page.sync="currentPage"
      ...
      <b-table-column field="porPagar" label="Por pagar Dólares" sortable v-slot="props">
        <span class="has-text-danger has-text-weight-bold"> ${{ formatoMontoLocal(props.row.porPagar) }}</span>
      </b-table-column>

      <b-table-column field="porPagar" label="Por pagar Bolívares" sortable v-slot="props">
        <span class="has-text-danger has-text-weight-bold">
          {{ props.row.idTasa ? `Bs. ${formatoMontoLocal(props.row.porPagar * tasa)}` : '-' }}
        </span>
      </b-table-column>
      ...
      <b-table-column field="abonar" label="Abonar" v-slot="props" v-if="can('ventas.realizar_abono')">
        <b-button type="is-primary" tag="router-link" :to="{ name: 'RealizarAbono', params: { id: props.row.id } }"
          v-if="props.row.porPagar > 0">
      ...
      <b-table-column field="acciones" label="Acciones" v-slot="props">
        ...
        <b-button type="is-danger" icon-left="trash-can-outline" @click="eliminarCuenta(props.row)"
          v-if="can('apartados.eliminar', 'cuentas.eliminar')">Eliminar</b-button>
      </b-table-column>
    </b-table>
  </section>
</template>
...
export default {
  props: ["datos", "printHref", "formatoMonto", "filtros", "tasa"],
