<template>
	<b-table :data="listaProductos">
		...
		<b-table-column field="precio" label="Precio" v-slot="props">
			...
			<b-select class="wide" icon="tag-multiple" v-model="props.row.precio" required @input="$emit('precioCambiado')"
				v-if="!props.row.mayoreoAplicado">
		...
		<b-table-column field="cantidad" label="Cantidad" v-slot="props">
			<div style="display: flex; align-items: center; gap: 1rem;">
				<b-field class="mb-0">
					<b-numberinput @input="aumentar(props.row)" min="0.1" step="0.01" :max="props.row.existencia"
						v-model="props.row.cantidad" style="width: 13em">
					</b-numberinput>
				</b-field>
				<span>
...
export default {
	...
	methods: {
		...
		precios (producto)
		{
			const { precioVenta, precioVenta2, precioVenta3, precioVenta4, precioVenta5, precioVenta6, precioVenta7 } = producto
			return [precioVenta, precioVenta2, precioVenta3, precioVenta4, precioVenta5, precioVenta6, precioVenta7]
